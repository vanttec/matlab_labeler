...
Labels = load('gangster_det_labels.mat');
...
Yolo_Coords = table('Size', [height(Label_Coords), 4], 'VariableTypes', {'double', 'double', 'double', 'double'}, 'VariableNames', {'X', 'Y', 'W', 'H'});
 ...
 for k=1:height(FilesTable)
    ...
    if not(isempty(Label_Coords.gangster{k,1}))
        Yolo_Coords.X(k) = Label_Coords{k,1}{1,1}(1);
        Yolo_Coords.Y(k) = Label_Coords{k,1}{1,1}(2);
        Yolo_Coords.W(k) = Label_Coords{k,1}{1,1}(3);
        Yolo_Coords.H(k) = Label_Coords{k,1}{1,1}(4);
    end
 end
...
for iK = 1 : height(Yolo_Coords)
    ...
    if Yolo_Coords{iK, 'H'} ~= 0
        %Then we verify if the .txt already exists
        if isfile(currTxtName)
    ...
    fprintf(TrainDataOut, '%d\t', labelIdentifier);
