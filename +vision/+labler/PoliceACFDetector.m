...
classdef PoliceACFDetector < vision.labeler.AutomationAlgorithm
    ...
    properties(Constant)
        ...
        Description = 'This is a custom automation algorithm to automatically label police bouys for a dataset froom RoboSub.';
    ...
    methods
        ...
        %
        function flag = supportReverseAutomation(~)
            flag = false ;
        end
        ...
        function isValid = checkLabelDefinition(~,labelDef)
            ...
            %Ony Rectagle ROIlabel definition are valid for gangster
            %Detector
        ...
        function isReady = checkSetup(algObj)
            ...
            %Check that a label is selected before automation begins
    ...
    methods
        ...
        function initialize(algObj,~)
            ...
            data = load('police_detector.mat');
            algObj.PoliceDetector = data.detector;  
        ...
        function autoLabels = run(algObj, I)
            ...
            autoLabels = [];
            [bboxes,scores] = detect(algObj.PoliceDetector,I,...
               'SelectStrongest',true);
            [~,idx] = max(scores);
            
            %[selectedBox,selectedScore] = selectStrongestBbox(bboxes,scores);
            %keeps detections above a certain threshold
            %detectionsToKeepIdx = (selectedScore > algObj.ScoreThreshold);
            selectedBox = bboxes(idx,:);
            
            if ~isempty(selectedBox)
                autoLabels.Name = algObj.SelectedLabelName;
                autoLabels.Type = labelType.Rectangle;
                autoLabels.Position = selectedBox;
            else
                autoLabels = [];
            end
